        ...
        for i in range(self.M):
            for j in range(self.N):
                for k in range(self.O):
                    ...
                    self.__transformedImage[i][j][k] = s[self.__originalImage[i][j][k]]
...
#Opcional Step: Show the results------------------------------------------------------
    def __showResults(self, histrOriginalImage, histrTransformedImage, s):
        ...
        imageComparison = np.hstack((self.__originalImage, self.__transformedImage))
...
#Final Step --------------------------------------------------------------------
    ...
    def calculate(self, image, results=False):
        ...
        self.__transformedImage = np.zeros(self.__originalImage.shape, dtype=np.uint8)
        self.M, self.N, self.O = self.__originalImage.shape
        ...
        histrOriginalImage = cv2.calcHist([self.__originalImage],[self.O - 1],None,[256],[0,256])
        ...
        if results == True:
            histrTransformedImage = cv2.calcHist([self.__transformedImage],[self.O - 1],None,[256],[0,256])
